    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto">
        <div className="text-center mb-14 sm:mb-20">
          <div
            ref={badgeRef}
            className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full bg-slate-500/5 dark:bg-white/5 border border-slate-500/10 dark:border-white/10 text-[10px] sm:text-xs font-bold uppercase tracking-widest text-(--text-muted) mb-4 sm:mb-5 shadow-sm transition-colors"
          >
          ...
          <h2
            ref={headingRef}
            className="text-3xl sm:text-5xl md:text-6xl font-black tracking-tight leading-tight mb-4 text-(--text-main) transition-colors duration-500"
          >
          ...
          <p
            ref={subRef}
            className="text-(--text-muted) text-sm sm:text-lg max-w-2xl mx-auto font-light leading-relaxed transition-colors duration-500"
          >
        ...
        <div className="wc-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 mb-20">
          {reasons.map((r) => (
            <div
              key={r.title}
              className="wc-card group relative flex flex-col gap-5 p-8 rounded-[2.5rem] bg-(--bg-main) border border-(--stat-card-border) transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:border-primary/20 overflow-hidden"
              onMouseEnter={(e) => (e.currentTarget.style.boxShadow = `0 24px 48px -12px ${r.glow}`)}
              onMouseLeave={(e) => (e.currentTarget.style.boxShadow = "none")}
            >
              <div
                className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl shadow-sm bg-slate-500/5 dark:bg-white/5"
                style={{ borderColor: `${r.color}20`, borderWidth: "1px" }}
              >
                {r.icon}
              </div>
        ...
        <div
          ref={ctaRef}
          className="relative flex flex-col lg:flex-row items-center justify-between gap-8 p-10 lg:p-14 rounded-[3rem] bg-tech-dark dark:bg-white/5 text-white shadow-2xl overflow-hidden border border-white/5"
        >
          ...
          <div className="relative text-center lg:text-left">
            ...
            <p className="text-slate-400 text-lg font-light max-w-xl">
              Join West Africa's boldest gathering of young builders and thinkers.
            </p>
          </div>
          <button
